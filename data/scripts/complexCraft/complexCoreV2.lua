...
local _debug = false
local _debugUpdate = 2 --Интервал работы цикла Update, если включен debug
local _secureSwitch = true --Несмотря на стремное название, всего лишь выключает (false) скрипту возможность загружать таблицы из памяти сервера :)
...
local locLines = {}
	...
	locLines['production_tooltip_streams'] = "Active production streams / Maximum number of streams of the current station"%_t
	...
	locLines['storage_tooltip_todelete'] = "On/off utilization mode.\n In this mode, the complex will try to accumulate this product from the stations indefinitely, while all goods over the storage limit will be deleted from stations"%_t
...
--GoodType
--TransferRate
--Priority (true - приоритет над алгоритмом потребления станциями)
--isActive
local tableSettings = {}
	table.insert(tableSettings,true) --isComplexActive
	table.insert(tableSettings,500) --baseTransferRate
...
function MX.initUI()
	...
	if _debug then
		...
		local testButton1 = tabSettings:createRoundButton(rectButtons[1], "data/textures/icons/SUBSYSPolarisationNanobots.png","analyse")
		testButton1.tooltip = "Пуск процедуры анализа"
		local testButton2 = tabSettings:createRoundButton(rectButtons[2], "data/textures/icons/SUBSYSPolarisationNanobots.png","buildRequestPacket")
		testButton2.tooltip = "Инициация пакета обновления"
		local testButton3 = tabSettings:createRoundButton(rectButtons[3], "data/textures/icons/SUBSYSPolarisationNanobots.png","DebugPurge")
		testButton3.tooltip = "Удалить батареи с комплекса и всех пристыкованных станций"
		local testButton4 = tabSettings:createRoundButton(rectButtons[4], "data/textures/icons/SUBSYSPolarisationNanobots.png","DebugClearMain")
		testButton4.tooltip = "Сброс основных таблиц"
		local testButton5 = tabSettings:createRoundButton(rectButtons[5], "data/textures/icons/SUBSYSEmergencyRepair.png","restoreOperate")
		...
		local testButton2 = tabSettings:createRoundButton(rectButtons[2], "data/textures/icons/MCXupdateLimit.png","buildRequestPacket")
		testButton2.tooltip = locLines['settings_button_forcerefresh']
		
		local testButton3 = tabSettings:createRoundButton(rectButtons[3], "data/textures/icons/SUBSYSPolarisationNanobots.png","DebugClearMain")
...
---------------------------------------------------------------------------------[Функции интерфейса и его генерации]
--Сканирует станции и провоцирует обновление главных таблиц
function MX.analyse()
	...
	local collectedStations = {} --захват всех станций, подходящих под условия (есть хотя бы один ресурс или товар)
	
	--------------------------[Сегмент производства/потребления]--------------------------
	--Алгоритм проверяет все пристыкованные объекты и заполняет временную таблицу результатов, выполняя отсев дубликатов
...
function MX.refreshMainTables(_analysisResult)
	--предварительный прогон, чтобы обнаружить неактивные(отсутствующие) сегменты и удалить их из таблицы
...
--создает интерактивную линию в интерфейсе production
...
--Выполняет полную обработку (обновление, удаление, поиск новых элементов) таблицы склада и отрисовывает элементы
function MX.renderStorageLine()
	--------------------------[Сегмент обработки таблицы склада]--------------------------
	local _goodsTable = {}
	
	--Отсекание
	if not(tabStorageScroller) then return end
	
	--Собирает список возможных уникальных товаров с производства
	...
	--Собирает список возможных уникальных товаров с потребления
	...
	--MX.TableSelfReport(_goodsTable,'UniqResoursesToStorage')
	
	--Удаляет неактуальный товар из текущей таблицы
	for _index,_rows in pairs(tableStorage) do
		if MX.isGoodUniq(_goodsTable,_rows[1]) then
			table.remove(tableStorage,_index)
			MX.DebugMsg('Good removed from table (wasnt presented in prod/cons tables): '..getGoodAttribute(_rows[1],'name'))
		end
	end
	
	--Обнаруживает неучтенный товар и добавляет его в таблицу
	...
	--------------------------[Сегмент отрисовки таблицы склада]--------------------------
	--отсекание
...
function MX.refreshInterface()
	...
	if _refreshAvialable then
		for _index,_rows in pairs(interfaceProduction) do
			...
			local _inactive = 0
			local _color = nil
			--Подсчитывает кол-во включенных и выключенных производств
			...
			--Составление списка производств и заполнение тултипа
			...
			--MX.TableSelfReport(_prodTable,'_prodTable')

			--установка производств
			--использует данные прошлых сегментов
			local _prodText = string.format("%s%i/%i",locLines['storage_label_production'],_active,#_thisProduction)
			_rows[3].text = _prodText
			
			--Цвет первичной иконки и иконка кнопки row[5]
			local _headIconText = ''
	...
	if _refreshAvialable then
		for _index,_rows in pairs(interfaceConsumption) do
			...
			_active = 0
			_inactive = 0
			--Подсчитывает кол-во включенных и выключенных потоков потребления
			...
			--Указывает соотношение доступных производств к потреблению.
			--local _consText = 'Обеспечено: '..tostring(_availableRoutes)..'/'..tostring(_active)
			local _consText = string.format("%s%i/%i",locLines['production_label_cons'],_availableRoutes,_active)
			_rows[3].text = _consText
			
			--Цвет первичной иконки
			if _active == 0 then
				_color = _colorR
				_rows[5].icon = _iconOff
	...
	if interfaceStorage and (#interfaceStorage>0) then
		for _index, _rows in pairs(interfaceStorage) do
			...
			--Лимит-окно не обновляется
		end
	end
end

function MX.onProductionClick(_button)
	--Определение индекса кнопки. Такой-то бред, пиздец :/
...
function MX.generateProdWindow(_stindex)
	...
	for _index,_rows in pairs(_table[2]) do
		...
		local _goodsOnStation = locLines['linegenerator_label_stationscargo']..tostring(CargoBay(_station.id):getNumCargos(_good))
		...
		--MX.TableSelfReport(_result,'_result')
...
--Выполняет обновление данных в открытой таблице подробностей по производству
function MX.generatedWindowsUpdate()
	------------------------------[Сегмент окна продукции]------------------------------
	if #MXtemp>0 then
		for _index,_row in pairs(MXtemp) do
			local _good = _row[3]
			local _stindex = _row[5]
			--объем товара
			local _goodsOnStation = locLines['linegenerator_label_stationscargo']..tostring(CargoBay(_row[4]):getNumCargos(_good))
			--статус иконки
			local _status = nil
...
--------------------------------------------------------------------------[Функции отправки/приема/удаления ресурсов]

--Формирует запрос на ресурсы, отталкиваясь от требований потребляющих станций и от возможностей производящих
function MX.buildRequestPacket()
	...
	--good
	--amount
	----------------------[Сегмент формирования запроса]----------------------
	--Сканирование путей запроса
	for _index,_row in pairs(tableStorage) do
		local _good = _row[1]
		...
		--Поиск ресурса на связанных станциях
		for _index2,_st in pairs(_entriesProdT) do
			if _amount>0 or _toDelete then
				...
				if _localAmount > 0 then --Проверка наличия ресурса
				
					if _amount<0 then _amount = 0 end
					
					if _localAmount >= _amount then --Проверка наличия запаса для одновременной передачи
						_resultRequest = {_st,_good,_amount,_toDelete}
						_amount = 0
					else --Частичная передача
						_resultRequest = {_st,_good,_localAmount,_toDelete}
						_amount = _amount - _localAmount
					end

					if _resultRequest then
						--Запрос в 0 единиц не направляется, если нет позиции toDelete
	...
	for _index,_row in pairs(tableStorage) do
		...
		if #_routes > 0 then
			--Выясняем текущий доступный запас данных ресов на комплексе
			local _requestTotal = CargoBay():getNumCargos(_good)
			--Выясняем потенциальный запрос с учетом всем потоков
			local _consumers = #_routes
			local _requestedByCons = _BV * _consumers
			--Уравниваем запрос и доступный объем ресурсов при необходимости
			if _requestedByCons>_requestTotal then
				_requestedByCons = _requestTotal
			end
			--Выполняем распределение ресурсов
			for _ind,_row2 in pairs(_routes) do
				--Вычисляем объем пакета из общего пула для одной станции
				local _PV = _requestedByCons / _consumers
				--Выполняем уточнение согласно объему товара, который уже имеется на станции
				local amountNeeded = _BV - CargoBay(_row2):getNumCargos(_good)
				if _PV > amountNeeded then
					_PV = amountNeeded
				end

				--Условное вычисление свободного места. Места должно хватать для распределения ресурса
				local _freeSpace = CargoBay(_row2).freeSpace
				if _PV*_goodSize > _freeSpace then
					_PV = _freeSpace / _goodSize - 1 --Маленький костыль :)
				end
				if _PV>1 then
					--Запись результата в активную таблицу
					local _result = {_row2,_good,_PV}
					--Запрос в 0 единиц не направляется
					if _result[3] > 0 then
						table.insert(requestConsumption,_result)
					end
					--Динамическое уменьшение пула и потребителей
					_requestedByCons = _requestedByCons - _PV
					_consumers = _consumers - 1
				else
					--MX.DebugMsg('_PV < 1, ошибка')
				end
			end
		end
	end
...
--Выполняет запрос на получение ресурса со станции
...
--Выполняет запрос передачу ресурса НА станцию
...
function MX.ServerSendResourses(_table)
	...
	--Проверка доступного ресурса
	local _resourseAvailable = CargoBay(_from):getNumCargos(_good)
	if _resourseAvailable<_amount then
		MX.DebugMsg('ServerSendResourses error: not enough goods('..getGoodAttribute(_good,'name')..', station: '..Entity(_from).name..') for transfer')
		_isRequestCorrect = false
	end
	...
	--Реакция на некорректный запрос
...
--Проверка на уникальность станции одновременно для таблиц production и consumption
...
--Синхронизирует информацию основных таблиц в системе клиент/сервер
...
--Выполняет обновление АКТИВНЫХ поставщиков/потребителей в таблице storage
...
--Возвращает индекс элемента, вызывающего функцию (кнопка, ТБ)
...
--Сканирует таблицу производства и возвращает кол-во производств указанного товара, либо 0, если данные пр-ва отсутствуют (устаревшее?)
...
--Проверяет скрипт на корректность для получения товаров
...
--Возвращает таблицу: ТОЛЬКО продукция станции (не возвращает саму станцию)
...
--Возвращает таблицу: требуемые ресурсы для станции
...
--Проверяет таблицу на наличие(false) или отсутствие(true) в ней указанного товара
...
--Проверяет таблицу на наличие данного типа товаров и возвращает таблицу относящихся станций
