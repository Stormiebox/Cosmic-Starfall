...
entities['combatgroupgeneral'] = {
	...
	--Содержимое
	{
		...
		{
			'desc', -- Тип элемента
			1.8, -- Высота (nil для iconname/mainlabel)
			"A special interface called by the corresponding button on the screen allows you to perform group management (player invitation, player kick, leader transfer) without using chat commands"%_t, -- Содержимое. Текст или путь к картинке.
		},
		{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			'data/textures/icons/uiCollapse.png', -- Содержимое. Текст или путь к картинке.
			"'Collapse' button"%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1, -- Высота (nil для iconname/mainlabel)
			"Minimizes the window, leaving the 'expand' button on its place, allowing you to expand it later"%_t, -- Содержимое. Текст или путь к картинке.
		},
		-- {
			-- 'iconinfo', -- Тип элемента
			-- nil, -- Высота (nil для iconname/mainlabel)
			-- 'data/textures/icons/uiUpdate.png', -- Содержимое. Текст или путь к картинке.
			-- "'Update' button"%_t -- Содержимое для icon info 
		-- },
		-- {
			-- 'desc', -- Тип элемента
			-- 1, -- Высота (nil для iconname/mainlabel)
			-- "Updates information in the active window"%_t, -- Содержимое. Текст или путь к картинке.
		-- },
		{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			'data/textures/icons/uiPlus.png', -- Содержимое. Текст или путь к картинке.
			"'Switch to adding' button"%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1, -- Высота (nil для iconname/mainlabel)
			"Opens a window with a list of online players who are not in your group"%_t, -- Содержимое. Текст или путь к картинке.
		},
		-- {
			-- 'iconinfo', -- Тип элемента
			-- nil, -- Высота (nil для iconname/mainlabel)
			-- 'data/textures/icons/uiPlayer.png', -- Содержимое. Текст или путь к картинке.
			-- "'Switch to Group Management' button"%_t -- Содержимое для icon info 
		-- },
		-- {
			-- 'desc', -- Тип элемента
			-- 1, -- Высота (nil для iconname/mainlabel)
			-- "Switches the main window to group management mode. Unavailable if you are not a group leader"%_t, -- Содержимое. Текст или путь к картинке.
		-- },
...
entities['combatgroupmanage'] = {
	...
	--Содержимое
	{
		...
		},
		{
			'desc', -- Тип элемента
			1, -- Высота (nil для iconname/mainlabel)
			"This window contains a list of all the players in the group, and also allows you to transfer leadership and kick players (including yourself). Unavailable functions will not be active"%_t, -- Содержимое. Текст или путь к картинке.
		},
		{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			locIcons['player'], -- Содержимое. Текст или путь к картинке.
			'Player status'%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1.2, -- Высота (nil для iconname/mainlabel)
			"Indicates the status of the player (offline, online, leader) and allows you to transfer the leader"%_t, -- Содержимое. Текст или путь к картинке.
		},
		{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			'data/textures/icons/kick.png', -- Содержимое. Текст или путь к картинке.
			"'Kick' button"%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1.2, -- Высота (nil для iconname/mainlabel)
			"Allows you to kick the player (available only to the leader). You can't kick players offline"%_t, -- Содержимое. Текст или путь к картинке.
		},
		{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			locIcons['cancel'], -- Содержимое. Текст или путь к картинке.
			"'Leave' button"%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1.2, -- Высота (nil для iconname/mainlabel)
			"Allows you to leave the group"%_t, -- Содержимое. Текст или путь к картинке.
		},
	},
}
	...
	{
		{
			'mainlabel', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			'data/textures/icons/acid-fog.png', -- Содержимое. Текст или путь к картинке.
		},
		{
			'desc', -- Тип элемента
			1.3, -- Высота (nil для iconname/mainlabel)
			"An interface element that is initially hidden. As soon as the effects of modules / environment from the Starfall mod begin to act on the ship, information about this effect will appear near the vanilla status icons"%_t, -- Содержимое. Текст или путь к картинке.
		},
	},
}
...
entities['asiinfo'] = {
	...
	--Содержимое
	{
		...
		},
		{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			'data/textures/icons/ui/ui_framesfull.png', -- Содержимое. Текст или путь к картинке.
			"Frame switcher"%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1.4, -- Высота (nil для iconname/mainlabel)
			"Allows you to include a frame on each system panel to improve visibility. The frame color is similar to the system color"%_t, -- Содержимое. Текст или путь к картинке.
		},
		{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			'data/textures/icons/ui/ui_moveinterface.png', -- Содержимое. Текст или путь к картинке.
			"Movement switcher"%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1.5, -- Высота (nil для iconname/mainlabel)
			"Allows you to unlock elements with which you can move panels around the screen. To do this, click on them and, holding the left mouse button pressed (the element will turn green), specify a new location"%_t, -- Содержимое. Текст или путь к картинке.
		},
		{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			'data/textures/icons/ui/ui_hideMainIcon.png', -- Содержимое. Текст или путь к картинке.
			"Main icon switcher"%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1.2, -- Высота (nil для iconname/mainlabel)
			"Allows you to enable and disable the display of the module icon on its panel"%_t, -- Содержимое. Текст или путь к картинке.
		},
		{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			'data/textures/icons/ui/ui_colors.png', -- Содержимое. Текст или путь к картинке.
			"Color switcher"%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1.4, -- Высота (nil для iconname/mainlabel)
			"Opens the color selection panel for the progress bars. Clicking on the button next to each bar resets the color to the original one"%_t, -- Содержимое. Текст или путь к картинке.
		},
				{
			'iconinfo', -- Тип элемента
			nil, -- Высота (nil для iconname/mainlabel)
			'data/textures/icons/ui/ui_default.png', -- Содержимое. Текст или путь к картинке.
			"Reset to Default"%_t -- Содержимое для icon info 
		},
		{
			'desc', -- Тип элемента
			1.3, -- Высота (nil для iconname/mainlabel)
			"Resets all panel settings to the original ones. After activation, you need to click on the confirmation button"%_t, -- Содержимое. Текст или путь к картинке.
		},
	},
}