...
function infoSystems.DebugMsg(_text)
	if _debug then
		include("cosmicvaultdebug").info("Cosmic Starfall", 'infoSystems|',_text)
	end
end
...
function infoSystems.SetMain(sysType,baseContainer,rUnit)
	...
	if not(sysType) then Debug('Failure: no sysType') return end
	...
	if sysSpellsTable then
		...
		for _,_rows in pairs(sysSpellsTable) do
			...
			local hModifier = 1 --for icon and name
...
--Creates labelBox for the weapons list
...
entities['bastionSystem'] = {
	...
	--Description
	{
	"A unique system designed to be installed on ships based on a shield. Modifies the ship's shields, reducing their volume, but in return increases their charging speed and reduces the time before it starts.\nActive Bastion systems allow you to reduce damage to the shield at the cost of reducing weapons fire rate (while repairing the hull), accumulate a charge from the damage received to replenish the shield, make it temporarily impenetrable, and also instantly destroy hostile torpedoes for a short time"%_t,
	...
	{
		{
			...
			"Activating the module increases the shield's resistance to all damage, and also slowly restores the ship's hull. During the operation of the module, the rate of fire of all weapons drops"%_t,
		...
		
		{
			...
			"Plasma, energy and electrical damage received by the shield accumulates a charge that can be used to restore shields"%_t,
		...
		
		{
			...
			"Activation makes the shields impenetrable for the duration of the module. Also, for a short time after activation, the delay before regeneration after receiving damage is significantly reduced"%_t,
		...
		
		{
			...
			"During operation, your ship will automatically and instantly shoot down all torpedoes that are aimed at it, as well as all hostile torpedoes within the range of the module. Anti-aircraft weapons are not required for operation"%_t,
	...
	{
		{
			...
			string.format("Activating the module burns the ship's energy very quickly, restoring the hull to all allied ships of the players. This effect also works on the ship itself, restoring the increased volume of the hull to it. Wave activation disables %s and %s"%_t,getSubtechName('macrofieldprojector',2),getSubtechName('macrofieldprojector',3)),
		...
		
		{
			...
			1.2 -- modifier for description window height from standard
		},
		
		{
			...
			'Forms a link between your ship and the target. The module will gradually syphon the shields in both directions, trying to equalize their percentage. It does not consume energy and can work independently of the other three modules./nWorks only on ships owned by players!'%_t,
...
entities['repairDrones'] = {
	...
	--Description
	{
	"Designed for combat vessels based on the hull. When installed, it slightly increases its volume and provides a unique bonus in the form of accelerated repairs when the hull is at a critical level. Active systems allow you to accelerate repairs in combat, increase the chances of surviving in a dangerous situation and restore hull faster out of combat"%_t,
	1.1, -- modifier for description window height from standard
	},
	--Passive bonuses / penalties
	{
	...
	{
		{
			...
			"Activation of this system restores hull for a limited time. However, this system will not restore hull if its volume exceeds 50%"%_t,
		...
		
		{
			...
			"Gradually restores the hull for a long time, works even in the negative environment of rifts. Receiving any damage or repairs with the help of repair lasers interrupts the operation of the module and reduces the remaining recharge time by 60%"%_t,
		...
		
		{
			...
			"Activation puts the module in standby mode for 20 seconds. If during this time hull durability falls below a certain value, the module turns on, simultaneously restoring a certain percentage of hull and accelerates passive repair three times"%_t,
	...
	{
		{
			...
			"Activation puts the module into standby mode. As soon as the ship begins to calculate the jump route, the module is activated and begins to quickly restore shields, while the rate of fire of the weapons will be seriously reduced before the jump"%_t,
		...
		
		{
			...
			"The activation of the module greatly accelerates the recharge of the hyperdrive, causing damage to the ship's hull during operation. For the module to work, it is necessary that the volume of the ship's shields fall below a certain threshold"%_t,
		...
		
		{
			...
			"Activation requires additional hyperdrive charging (cannot be performed if the hyperdrive is still charging) and increases the allowable jump range by more than twice. Adds extra time to recharge after performing a jump, depending on the range bonus provided."%_t,
...
entities['pulseTractorBeamGenerator'] = {
	...
	--Description
	{
	"Does not provide any passive bonuses. It is equipped with only one active system, which gradually expands the radius of the attracting beam to large values, allowing you to collect loot in a huge radius during the action"%_t,
	1, -- modifier for description window height from standard
	},
	--Passive bonuses / penalties
	{
	...
	{
		{
			...
			"During operation, every two seconds it makes an impulse that increases the range of the attracting beam by a fixed value. The number of pulses depends on the rarity level of the system. At the end of the work, the beam range returns to the initial value"%_t,
