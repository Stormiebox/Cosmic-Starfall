...
--=================================[Создание/отрисовка интерфейса]=================================
function combatGroup.CreateInterface()
	if onServer() then return end
	
	--Управляющие переменные
	local mainWidth = rUnit*4 --Ширина окна
	local mainHeight = rUnit*3 --Высота окна
	local topContHeight = rUnit*0.7 --Высота верхней строки
	
	local res = getResolution()
	local mainAnchor = vec2(res.x*0.035,res.y*0.22)
	
	local bottomContainer = { --Нижний контейнер (управление группой)
		0, topContHeight,
		mainWidth,
		mainHeight - topContHeight
	}
	local rightContainer = { --Правый контейнер (добавление)
		mainWidth + rUnit * 0.4, 0,
		mainWidth,
		mainHeight
	}
	local mainLabel = { --Квадрат наименования
		rUnit * 0.3, rUnit * 0.05, --якорь
		rUnit*2, --ширина
		rUnit *0.95 --высота
	}
	local collapseButton = { --Кнопка сворачивания (разворачивания)
		rUnit * -0.7,rUnit * 0.05,
		rUnit * 0.6,
		rUnit * 0.6
	}
	local addButton = { --Кнопка добавления
		collapseButton[1],collapseButton[2] + collapseButton[4] + rUnit * 0.2,
		rUnit * 0.5,
		rUnit * 0.5
	}
	local collapseLeftButton = { --Кнопка сворачивания-меню-добавления
		mainWidth - rUnit*0.5,rUnit * 0.05,
		rUnit * 0.5,
		rUnit * 0.5
	}
	local acceptinvitelabel = { --label принятия приглашения
		rUnit * 0.3, rUnit * 0.2,
		rUnit * 4,
		rUnit * 0.6
	}
	local acceptinvitebutton = { --Кнопка принятия приглашения
		rUnit * 0.3, acceptinvitelabel[2] + acceptinvitelabel[4],
		rUnit * 0.5,
		rUnit * 0.5
	}
	local declineinvitebutton = { --Кнопка отказа от приглашения
	...
	mainContainer = Hud():createContainer(containerRect)
		--mainContainer:createFrame(Rect(mainContainer.size))
		
	--Создание внутренних зон
	...
	--Кнопка "свернуть добавление"
	buttons['collapseLeft'] = innerContainers['top']:createRoundButton(getrect(collapseLeftButton),locIcons['collapseleft'],'collapseAdd')
	...
	--Заполнение зоны инвайта
	--labelFontSize = rUnit * 0.3
	--Надпись
	local invitelabel = innerContainers['bottom_invite']:createLabel(getrect(acceptinvitelabel),locLines['label_caption_invited'],labelFontSize)
	--Кнопка "принять"
	buttons['acceptinvite'] = innerContainers['bottom_invite']:createRoundButton(getrect(acceptinvitebutton),locIcons['sent'],'AcceptInvite')
	--Кнопка "отказаться"
	buttons['declineinvite'] = innerContainers['bottom_invite']:createRoundButton(getrect(declineinvitebutton),locIcons['cancel'],'interruptInvite')
	
		
	--Инициирующее сворачивание
	self.Collapse()
	...
	--Инициализация хуков и интерфейса
...
function combatGroup.RenderGroup()
	...
	--Отсекает рендер и открывает кнопку "добавить", если игрок не в группе 
	if isGroup == nil then
		buttons['add']:show()
		return
	end

	--Если группа существует - генерирует ее линии
	local party = Player().group
	local partyGuys = {party:getPlayers()}
	local isSelfLeader = (Player().index == party.leader)
	
	--Открывает кнопку "добавить"
	...
	for _,_rows in pairs(partyGuys) do
		...
		Debug(sf("(%s)\nisLeader - %s\nisOnline - %s\nisSelf - %s,\nisSelfLeader - %s",name,tostring(isLeader),tostring(isOnline),tostring(isSelf),tostring(isSelfLeader)))
		
		--Выборочное включение элементов
		--===[Блок кнопки лидера]===--
		--Показать кнопку
...
function combatGroup.GenerateGroupLine(yPos)
	...
	--Переменные
	local yPadding = rUnit * 0.05 + yPos --смещение всей линии по вертикали
	local fontSize = rUnit * 0.23
	local localSize = rUnit * 0.6
	
	local leaderButton = { --Кнопка/иконка лидера
		rUnit * 0.1,yPadding,
		localSize,
		localSize
	}
	local nameLabel = { --Имя игрока
		leaderButton[1]+leaderButton[3],yPadding + localSize*0.35,
		localSize*4,
		localSize * 1.35
	}
	local kickButton = { --Кнопка кик/ливнуть
	...
	--Предзаглушка всех элементов строки
	for _,_rows in pairs(uiElems) do
		_rows:hide()
	end
	
	--возврат таблицы и yPos
...
--Server->Client поиск игроков онлаен, кроме того инициируется хуками для обновления инфы
function combatGroup.getOnlinePwayers(_table)
	if onServer() then
		--Сканирование информации и синхронизация
...
function combatGroup.getPwayersToInvite()
	...
	-- --Если группы нет, не проверяет группу
	if party == nil then
...
--Эту функцию вызывает сервер для создания пришлашения
function combatGroup.remoteInvite()
	if onServer() then
		invokeClientFunction(Player(),'remoteInvite')
	else
		invintationFlag = 40 --период мигания кнопки
...
--Эту функцию вызывает сервер для удаленного кика
...
--=================================[Служебные]=================================
--Выполняет имитацию команды чата
...
--Просто способ создать квадрат по принципу якорь-точка
...
--Проверяет наличие имени/индекса в списке имен онлайн
