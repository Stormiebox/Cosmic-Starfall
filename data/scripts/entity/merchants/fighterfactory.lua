...
function FighterFactory.makeFighter(type, plan, turret, sizePoints, durabilityPoints, turningSpeedPoints, velocityPoints)
    ...
    --Assignment of the global light tourettes bonus modifier
    ...
    if turret then
        ...
        for _, weapon in pairs({ turret:getWeapons() }) do
            ...
            --Range modification unique to different turrets
...
function FighterFactory.refreshPointLabels(plan, turret)
    ...
    local modifiedSize, modifiedDurability, modifiedTurningSpeed, modifiedVelocity = FighterFactory.addMaterialBonuses(
    material, sizePoints, durabilityPoints, turningSpeedPoints, velocityPoints)
    ...
    local fighter = FighterFactory.makeFighter(selectedType, plan, turret, sizePoints, durabilityPoints,
        turningSpeedPoints, velocityPoints)
...
function FighterFactory.createFighter(type, plan, turretIndex, sizePoints, durabilityPoints, turningSpeedPoints,
    ...
    if plan.numBlocks > 200 then
        player:sendChatMessage("Fighter Factory" % _t, ChatMessageType.Error,
            "Only plans with 200 blocks or less allowed." % _t)
        return
    end
    ...
    if turret then
        ...
        if turret.blackMarketDLCOnly and not player.ownsBlackMarketDLC then
            player:sendChatMessage("Fighter Factory" % _t, ChatMessageType.Error,
                "You must own the Black Market DLC to build this fighter." % _T)
            return 0
        end
        --If the weapon belongs to the heavy class -cutting off
        if isTurretHeavy(turret, nil) then
            Debug('createFighter: heavy eroro')
            player:sendChatMessage("Fighter Factory" % _t, ChatMessageType.Error,
                'Нельзя установить тяжелое вооружение на истребители')
            return 0
        end
        --If the weapon belongs to the "main caliber" class -cutting off
        if isTurretMC(turret, nil) then
            Debug('createFighter: MC eroro')
            player:sendChatMessage("Fighter Factory" % _t, ChatMessageType.Error,
                'Орудия главного калибра нельзя устанавливать на истребители!')
            return 0
        end
        ...
        if turret.intoTheRiftDLCOnly and not player.ownsIntoTheRiftDLC then
            player:sendChatMessage("Fighter Factory" % _t, ChatMessageType.Error,
                "You must own the Into The Rift DLC to build this fighter." % _T)
            return 0
        end

        if turret.ancient then
            player:sendChatMessage("Fighter Factory" % _t, ChatMessageType.Error,
                "This turret can't be integrated into a fighter." % _T)
            return 0
        end
    end
    ...
    local fighter = FighterFactory.makeFighter(type, plan, turret, sizePoints, durabilityPoints, turningSpeedPoints,
        velocityPoints)
    local boughtFighter = SellableFighter(fighter)
