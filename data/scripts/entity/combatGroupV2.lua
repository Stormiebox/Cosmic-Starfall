...
function combatGroup.executeServerCmd(_type, _cmd, _name)
    if onClient() then
        ...
        if _type == 'invite' then
            ...
            local targetPlayer = Galaxy():findPlayer(_name)
            if targetPlayer then
                invokeFactionFunction(targetPlayer.index, false, 'combatGroupV2', 'remoteInvite', _name)
            end
        end
        ...
        if _type == 'kick' then
            local targetPlayer = Galaxy():findPlayer(_name)
            if targetPlayer then
                invokeFactionFunction(targetPlayer.index, false, 'combatGroupV2', 'remoteKick', _name)
            end
        end
