...
--Создает стандартное окно на слое Hud
...
--Создает обновленное окно алерта в ЛЕВОМ углу экрана
function Neltharaku.CreateAlertV2(_isActive)

	--Базовые переменные
	local res = getResolution()
	local _unit = res.y * 0.07 / 4
	local _shrinkMod = 0.3 --Сжимание квадрата основной кнопки
	
	--Вычисление позиции алерта
	local _size = _unit * 3
	...
	--Создание кнопки / текстового поля
...
function Neltharaku.ExpandAlert(_elements)
	...
	--Установка размера инфо и ее подсветка
...
--Возвращает таблицу с Rect'ами для элементов в линии согласно полученной информации
...
--Перемещает кнопку закрытия согласно размеру окна
function Neltharaku.UIplaceCloseButton(_button,_window,_position)
	...
	local _anchorPoint = vec2(_windowTopRight.x - _sizePadding - _size - _positionPadding, _windowTopRight.y + _negativePadding)
	local _secondPoint = vec2(_anchorPoint.x + _size,_anchorPoint.y + _size)
	
	local _rect = Rect(_anchorPoint,_secondPoint)
	
	--Установка нового расположения
	_button.rect = _rect
end

------------------------------------------[Превращения и вычисления]--------------------------------
...
--Рисует рамку внутри window из фреймов. Если отправить color - раскрасит ее
...
--Генерирует фрейм, который совпадает с квадратом элемента
...
--Создает контейнер в контейнере
